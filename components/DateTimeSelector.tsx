...
export default function DateTimeSelector({ data }: DateTimeSelectorProps) {
  const { list, selectedIndex, setSelectedIndex, isLoading, handleAdd, handleRemove, handleToggleReserved } =
  ...
  // Filter out past dates on mount
  useEffect(() => {
    const today = new Date()
    today.setHours(0, 0, 0, 0)
    // Note: The filtering should happen before passing data to this component
  }, [])
  ...
  return (
    <div className='flex w-full max-w-2xl flex-col gap-6 rounded border bg-white p-6 shadow'>
      <div className='space-y-4'>
        ...
        <CalendarForm onAdd={handleAddWithToast} isLoading={isLoading} />
      </div>
